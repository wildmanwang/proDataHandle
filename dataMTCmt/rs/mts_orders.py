...
class Handler(BaseHandler):
    ...
    def on_start(self):
        ...
        try:
            ...
            for rcStore in rsStore:
                ...
                storeVar["headers_settle"] = self.headers_order.copy()
    ...
    def index_page(self, response):
        ...
        while iNum > 0:
            ...
            self.crawl(sUrl + "#" + sItag, headers=response.save["headers_order"], itag=sItag, age=2*60, retries=2, callback=self.detail_page_order)
    ...
    def detail_page_order(self, response):
        ...
        self.crawl(sUrl_prepare, headers=response.save["headers_order"], age=0, retries=2, callback=self.detail_page_prepare)
        sUrl_delivery += r"%5D&region_id={region_id}&region_version={region_version}".format(
            region_id=response.save["varDict"]["region_id"],
            region_version=response.save["varDict"]["region_version"]
        )
        self.crawl(sUrl_delivery, headers=response.save["headers_order"], age=0, retries=2, callback=self.detail_page_delivery)
        # 注释以下3行代码，暂不需要抓取结算数据
        ...
        # self.crawl(sUrl_settle + "#" + sItag, method="POST", data={"chargeInfo": sData}, headers=response.save["headers_settle"], itag=sItag, age=0, retries=2, callback=self.detail_page_settle)
